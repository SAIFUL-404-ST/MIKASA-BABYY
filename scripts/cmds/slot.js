...
function formatMoney(amount) {
  ...
  let matched = false;
  for (let u of units) {
    if (amount >= u.v) { out = fancy((amount / u.v).toFixed(2)) + u.s; matched = true; break; }
  }
  if (!matched) out = fancy(Math.floor(amount).toLocaleString());
  return (negative ? "-" : "") + out;
}
...
function ensureStats(user, now) {
  if (!user.data.slotStats) user.data.slotStats = freshStats(now);
  const stats = user.data.slotStats;
...
// 🖼️ Build the "slot info" card Baby
function buildInfoCard(stats) {
  ...
  return (
    "🎰 " + fancy("SLOT MACHINE — YOUR INFO BABY") + "\n" +
    "━━━━━━━━━━━━━━━━━━━\n\n" +
    fancy("📊 Your Database:") + "\n" +
...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData, role }) {
    ...
    // 📌 Simple rules/usage message Baby — shown on first play AND anytime "slot" is typed with no amount
    const rulesMsg =
      `🎀 𝐒𝐋𝐎𝐓 𝐌𝐀𝐂𝐇𝐈𝐍𝐄 𝐁𝐀𝐁𝐘\n` +
      `━━━━━━━━━━━━━━━━━━━\n\n` +
      fancy(`📌 How To Play:\n`) +
    ...
    // 🏆 slot top Baby (leaderboard by net profit, best-effort)
    if (sub === "top") {
      ...
      try {
        ...
        const ranked = all
          .filter(u => u.data && u.data.slotStats && u.data.slotStats.totalGames > 0)
          .map(u => {
        ...
        return api.sendMessage(
          fancy("🏆 SLOT TOP 10 — NET PROFIT BABY\n━━━━━━━━━━━━━━━━━━━\n") + lines,
          threadID, messageID
        );
      } catch (e) {
    ...
    {
      // Normal game:
      // 3% Rare Jackpot (triple) | 37% Double | 60% Lose
    ...
    if (isJackpot) {
      ...
      const statusText = isWin ? fancy("Won") : fancy("Lost");
      resultMsg =
        ">🎀\n" +
